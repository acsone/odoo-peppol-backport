...
class AccountEdiProxyClientPeppolUser(models.Model):
    ...
    def _auto_init(self):
        ...
        if not index_exists(
            self.env.cr,
            "account_edi_proxy_client_peppol_user_unique_active_company_proxy",
        ):
            self.env.cr.execute(
                """
        ...
        if proxy_error:
            ...
            if error_code == "invalid_signature":
                raise AccountEdiProxyError(
                    error_code,
                    _(
                        "Invalid signature for request. This might be due to another connection to odoo Access Point "
    ...
    def _renew_token(self):
        ...
        try:
            with self.env.cr.savepoint(flush=False):
                self.env.cr.execute(
                    "SELECT * FROM account_edi_proxy_client_peppol_user WHERE id IN %s FOR UPDATE NOWAIT",
                    [tuple(self.ids)],
                )
