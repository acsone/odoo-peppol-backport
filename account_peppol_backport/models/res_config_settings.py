...
class ResConfigSettings(models.TransientModel):
    ...
    account_peppol_verification_code = fields.Char(
        related="account_peppol_edi_user.peppol_verification_code", readonly=False
    )
    ...
    def _call_peppol_proxy(self, endpoint, params=None, edi_user=None):
        ...
        if not edi_user:
            edi_user = self.company_id.account_edi_proxy_client_peppol_ids.filtered(
        ...
        if "error" in response:
            ...
            error_message = response["error"].get("message") or response["error"].get(
                "data", {}
            ).get("message")
            raise UserError(
    ...
    def _onchange_account_peppol_endpoint(self):
        if self.account_peppol_endpoint:
            self.account_peppol_endpoint = "".join(
                char for char in self.account_peppol_endpoint if char.isalnum()
            )
    ...
    def _compute_account_peppol_mode_constraint(self):
        mode_constraint = (
            ...
            .sudo()
            .get_param("account_peppol.mode_constraint")
        )
        trial_param = (
            self.env["ir.config_parameter"].sudo().get_param("saas_trial.confirm_token")
        )
        self.account_peppol_mode_constraint = (
            trial_param and "demo" or mode_constraint or "prod"
        )
        ...
        for config in self:
            if not config.account_peppol_edi_user and config.account_peppol_edi_mode:
                self.env["ir.config_parameter"].sudo().set_param(
                    "account_peppol.edi.mode", config.account_peppol_edi_mode
                )
                return
    ...
    def _compute_account_peppol_edi_user(self):
        for config in self:
            config.account_peppol_edi_user = (
                config.company_id.account_edi_proxy_client_peppol_ids.filtered(
                    lambda u: u.proxy_type == "peppol"
                )
            )
    ...
    def _compute_account_peppol_endpoint_warning(self):
        for config in self:
            if (
                ...
                config.account_peppol_endpoint_warning = _(
    ...
    def button_create_peppol_proxy_user(self):
        ...
        if not self.account_peppol_phone_number:
            raise ValidationError(
                _("Please enter a mobile number to verify your application.")
            )
        ...
        edi_identification = edi_proxy_client._get_proxy_identification(
            company, "peppol"
        )
        company.partner_id._check_peppol_eas()

        if (
            participant_info := company.partner_id._check_peppol_participant_exists(
                edi_identification, check_company=True
            )
            ...
            if isinstance(participant_info, str):
                error_msg += _(
                    "The Peppol service that is used is likely to be %s.",
                    participant_info,
                )
    ...
    def button_update_peppol_user_data(self):
        ...
        if (
            not self.account_peppol_contact_email
            or not self.account_peppol_phone_number
        ):
    ...
    def button_cancel_peppol_registration(self):
        ...
        if self.account_peppol_proxy_state == "active":
            raise UserError(
                _(
                    "Can't cancel an active registration. Please request a migration or deregister instead."
                )
            )

        if self.account_peppol_proxy_state in {
            "not_registered",
            "rejected",
            "canceled",
        }:
            raise UserError(
                _(
                    "Can't cancel registration with this status: %s",
                    self.account_peppol_proxy_state,
                )
            )
    ...
    def button_deregister_peppol_participant(self):
        ...
        if self.account_peppol_proxy_state != "active":
            raise UserError(
                _(
                    "Can't deregister with this status: %s",
                    self.account_peppol_proxy_state,
                )
            )
        ...
        ]._cron_peppol_get_message_status()
        self.env[
            "account_edi_proxy_client_peppol.user"
