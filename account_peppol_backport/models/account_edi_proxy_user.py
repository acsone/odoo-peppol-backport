...
class AccountEdiProxyClientPeppolUser(models.Model):
    ...
    def _make_request(self, url, params=False):
        assert (
            not self.proxy_type or self.proxy_type == "peppol"
        ), "This proxy object should only be used for Peppol requests."
    ...
    def _make_request_peppol(self, url, params=False):
        ...
        try:
            ...
            if (
                e.code == "no_such_user"
                and not self.active
                and not self.company_id.account_edi_proxy_client_peppol_ids.filtered(
    ...
    def _peppol_get_message_status(self):
        ...
        if need_retrigger:
            self.env.ref(
                "account_peppol_backport.ir_cron_peppol_get_message_status"
            ).method_direct_trigger()
        ...
        edi_users = self.search(
            [
                (
                    ...
                    "in",
                    ["pending", "not_verified", "sent_verification"],
                ),
                ("proxy_type", "=", "peppol"),
            ]
        )
    ...
    def _peppol_get_participant_status(self):
        for edi_user in self:
            ...
            if proxy_user["peppol_state"] in state_map:
                edi_user.company_id.account_peppol_proxy_state = state_map[
                    proxy_user["peppol_state"]
                ]
    ...
    def _peppol_send_document(self, invoice, xml_string: bytes, xml_filename: str):
        ...
        try:
            ...
            if response.get("error"):
                ...
                invoice.peppol_move_state = "error"
                invoice._message_log(
                    body=_("Peppol error: %s", response["error"].get("message"))
                )
            else:
                ...
                if message_uuid:
                    ...
                    if (
                        ...
                        .sudo()
                        .get_param("account_peppol_backport.log_sent_xml")
                    ):
                    ...
                    invoice._message_log(
                        ...
                        attachment_ids=[sent_xml_attachment.id]
                        if sent_xml_attachment
                        else [],
                    )
                else:
                    invoice.peppol_move_state = "error"
                    invoice._message_log(
                        body=_("Peppol proxy did not return a message uuid.")
                    )
