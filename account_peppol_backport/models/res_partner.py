...
class ResPartner(models.Model):
    ...
    def _auto_init(self):
        ...
        if not column_exists(
            self.env.cr, "res_partner", "account_peppol_is_endpoint_valid"
        ):
            create_column(
                self.env.cr,
                "res_partner",
        ...
        if not column_exists(
            self.env.cr, "res_partner", "account_peppol_validity_last_check"
        ):
            create_column(
                self.env.cr,
                "res_partner",
    ...
    @api.model
    def _check_peppol_participant_exists(
        self, edi_identification, check_company=False, ubl_cii_format=False
    ):
        ...
        if (
            edi_identification != participant_identifier
            or "hermes-belgium" in service_href
        ):
            ...
            with contextlib.suppress(
                ...
                if response.status_code == 200:
                    ...
                    access_point_contact = access_point_info.findtext(
                        ".//{*}TechnicalContactUrl"
                    ) or access_point_info.findtext(".//{*}TechnicalInformationUrl")
    ...
    def _check_document_type_support(self, participant_info, ubl_cii_format):
        service_metadata = participant_info.find(
    ...
    def _compute_is_peppol_edi_format(self):
        for partner in self:
            partner.is_peppol_edi_format = partner.ubl_cii_format not in (
                False,
                "facturx",
                "oioubl_201",
                "ciusro",
    ...
    @api.depends(
        "account_peppol_is_endpoint_valid", "account_peppol_validity_last_check"
    )
            ...
            if not partner.account_peppol_validity_last_check:
                ...
                and (
                    participant_info := self._get_participant_info(
                        f"{partner.peppol_eas}:{partner.peppol_endpoint}".lower()
                    )
                )
                is not None
                and not partner._check_document_type_support(
                    participant_info, partner.ubl_cii_format
                )
            ):
    ...
    def button_account_peppol_check_partner_endpoint(self):
        ...
        if (
            ...
            if not self.account_peppol_is_endpoint_valid and self.peppol_eas in (
                "0208",
                "9925",
            ):
                inverse_eas = "9925" if self.peppol_eas == "0208" else "0208"
                inverse_endpoint = (
                    f"BE{self.peppol_endpoint}"
                    if self.peppol_eas == "0208"
                    else self.peppol_endpoint[2:]
                )

                if self._check_peppol_participant_exists(
                    f"{inverse_eas}:{inverse_endpoint}",
                    ubl_cii_format=self.ubl_cii_format,
                ):
