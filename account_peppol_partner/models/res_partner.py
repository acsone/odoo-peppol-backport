...
class ResPartner(models.Model):
        ...
        for partner in self:
            if partner.peppol_eas in ("0037", "0212", "0213", "0215"):
                raise ValidationError(
                    _(
                        "Peppol EAS codes 0037, 0212, 0213, 0215 are deprecated. Please use 0216 instead."
                    )
                )
            elif partner.peppol_eas == "9955":
                raise ValidationError(
                    _("Peppol EAS code 9955 is deprecated. Please use 0007 instead.")
                )
            elif partner.peppol_eas == "9901":
                raise ValidationError(
                    _(
                        "Peppol EAS code 9901 is deprecated. Please use a different Danish EAS code instead."
                    )
                )
    ...
    def _check_peppol_fields(self):
        for partner in self:
            if partner.peppol_endpoint and partner.peppol_eas:
                error = self._build_error_peppol_endpoint(
    ...
    def _compute_ubl_cii_format(self):
        ...
        for partner in self:
            ...
            if country_code in format_mapping:
                ...
                country_code in PEPPOL_DEFAULT_COUNTRIES and country_code in EAS_MAPPING
            ):
        ...
        for partner in self:
            ...
            if country_code in EAS_MAPPING:
                ...
                if (
                    ...
                    and partner[field]
                    and not partner._build_error_peppol_endpoint(
                        partner.peppol_eas, partner[field]
                    )
                ):
                ...
                if partner.peppol_eas not in eas_to_field.keys():
                    ...
                    for eas, field in eas_to_field.items():
                        if field and field in partner._fields and partner[field]:
                            if not partner._build_error_peppol_endpoint(
                                eas, partner[field]
                            ):
                                new_eas = eas
                                break
    ...
    def _compute_hide_peppol_fields(self):
        ...
        for partner in self:
            partner.hide_peppol_fields = (
                not partner.ubl_cii_format or partner.ubl_cii_format == "facturx"
            )
